export async function loadGraph(inputdata, denominazione_regione, toggle) {
    ...
    globalThis.$('#tooltip').html('<p class="line"> Numero totale di casi: ' + globalThis.d3.max(data, function (d) { return Math.max(d.totale_casi, d.dimessi_guariti); }) + 
                    '</p><p class="line2"> Numero totale di guariti: ' + globalThis.d3.max(data, function (d) { return Math.max(d.dimessi_guariti, d.dimessi_guariti); }) + '</p>')
